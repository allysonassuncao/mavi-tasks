...
- **Google:** TRÁFEGO = cliques; ENGAJAMENTO = impressões; VIDEO = visualizações TrueView. Nos demais, as **ações de conversão pelo nome** (sem acento, cada uma arredondada): conta `whats|phone|compra|lead|local|purch|cadastro|contato|inscreve|subscription|subs|inscritos|inscri|instala`, nunca `visualiza|viu|finali|checkout|cart|content|carri`, mais as **ligações dos anúncios** (`phone_calls`). Com página de captura da Make, só `whats|phone|local|compra|purch` (os leads vêm da Make). VENDA separa o funil pelo nome.
