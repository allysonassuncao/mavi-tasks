...
- **Mais fontes** (fase 3, migração `20261023090000_ai_sources`): arquivos do Drive (o worker baixa do GCS e lê PDF, Word, PowerPoint, Excel, CSV e texto em `api/_ai-extract.ts`, por página, slide ou planilha; imagens, PDFs escaneados e outros tipos entram só pelo nome e pela pasta), briefing e planos do Social Leads (acesso do produto contratado) e anotações e ciclos das Campanhas (só líderes). Os números das campanhas vêm ao vivo pela ferramenta `campaign_results`. As citações abrem o arquivo no Drive (`?arquivo=`), o plano no Social Leads e a campanha em Campanhas.
...
Para ligar: aplicar a migração (ela já coloca todo o histórico na fila), configurar `OPENAI_API_KEY` e `AI_WORKER_SECRET` na Vercel e fazer redeploy; depois, no banco, `insert into mavi_private.ai_config(url, secret) values ('https://<domínio>/api/ai', '<AI_WORKER_SECRET>')` e rodar `supabase/operations/schedule-ai-index.sql`. Testes: `npm run test:db:ai`, `npm run test:db:ai-assistant`, `npm run test:db:ai-sources` e `npx vitest run api/_ai.test.ts api/_ai-extract.test.ts`.
