...
- **Google:** TRÁFEGO = cliques; ENGAJAMENTO = impressões; VIDEO = visualizações TrueView. Nos demais, **todas as ações da coluna "Conversões" do Google** (as principais, cada uma arredondada), nunca as micro `visualiza|viu|finali|checkout|cart|content|carri`, mais as **ligações dos anúncios** (`phone_calls`). O MASO ainda exigia o nome numa lista (`lead`, `contato`, `whats`…): ação com outro nome contava zero e o analista lançava o número à mão. Com página de captura da Make, só `whats|phone|local|compra|purch` (a lista do MASO: os leads vêm da Make, e a conversão do formulário os contaria em dobro). VENDA separa o funil pelo nome. No histórico importado do MASO, as impressões do Google (que o MASO gravava em "alcance") foram para o lugar certo (migração `20261012090000_ad_google_maso_impressions`).
